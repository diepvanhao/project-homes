...
if (!empty($agent_id)) {
    $agent = $report->getGroupInfo($agent_id);
    $agent_name = $agent['group_name'];
} else {
    $agent_id = null;
    $agent_name = '';
}
...
$smarty->assign('users', $users = $report->getUsersByGroup($agent_id));
$smarty->assign('groups', $groups = $report->getAllGroups());
$smarty->assign('agent', $agent = $report->getGroupInfo($agent_id));
